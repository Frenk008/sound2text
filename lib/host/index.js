...
`, "utf8");
  ...
  disposers.push(
    ctx.webServer.register({
      kind: "exact",
      path: `${PREFIX}/status`,
      handler: (_req, res) => json(res, 200, { running: !!helper, model: model(), baseUrl: baseUrl(), hasKey: !!apiKey(), python: python(), device: cfg.device ?? process.env.S2T_DEVICE ?? "(default output)", lastError })
    })
  );
  disposers.push(
    ctx.webServer.register({
